"""Candidate-layer audit of short-term pre-entry overheating.

The pre-declared primary hypothesis is that a lower 20-session return through
the closed signal day should improve future and realized trade returns.  The
5-session return, distance to the 20-session high, and signal-day opening gap
are diagnostics only.  Comparisons are formed within each signal day, the
strategy enters on T+1, canonical development candidates are required,
holdout is rejected, and no portfolio layer is implemented.
"""
...
VERSION = "short_term_overheat_audit.v1"
SPLITS = ("train", "val", "test")
PRIMARY_FACTOR = "pre_entry_20d_return"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    "pre_entry_20d_return": {"larger_is_better": False, "role": "primary"},
    "pre_entry_5d_return": {"larger_is_better": False, "role": "diagnostic"},
    "distance_to_20d_high": {"larger_is_better": False, "role": "diagnostic"},
    "signal_day_gap": {"larger_is_better": False, "role": "diagnostic"},
}
...
def _safe_return(current: Any, previous: Any) -> float | None:
    ...
    if (
        ...
        or not np.isfinite(previous_value)
        or current_value <= 0
        or previous_value <= 0
    ):
        return None
...
def _overheat_features(
    frame: pd.DataFrame,
    signal_index: int,
) -> dict[str, Any]:
    """Compute causal pre-entry features through one closed signal bar."""
    result: dict[str, Any] = {factor: None for factor in FACTOR_SPECS}
    result.update(
        {
            "factor_assignment_available": False,
            "factor_error": None,
            "overheat_lookback_bars": signal_index + 1,
            "twenty_day_high_includes_signal_day": True,
            "entry_timing": "T+1",
        }
    )
    if signal_index < 0 or frame.empty or "close" not in frame.columns:
        ...
        return result
    if signal_index < 20:
        result["factor_error"] = "insufficient_20_session_return_history"
        return result

    limited = frame.iloc[: signal_index + 1]
    close = pd.to_numeric(limited["close"], errors="coerce")
    current_close = close.iloc[signal_index]
    primary = _safe_return(current_close, close.iloc[signal_index - 20])
    if primary is None:
        result["factor_error"] = "invalid_pre_entry_20d_return"
        return result

    pre_entry_5d = _safe_return(
        current_close,
        close.iloc[signal_index - 5],
    )

    distance_to_high = None
    if "high" in limited.columns:
        high_window = pd.to_numeric(
            limited["high"].iloc[signal_index - 19 : signal_index + 1],
            errors="coerce",
        ).dropna()
        if not high_window.empty:
            high_20 = float(high_window.max())
            distance_to_high = _safe_return(current_close, high_20)

    signal_day_gap = None
    if "open" in limited.columns:
        signal_day_gap = _safe_return(
            pd.to_numeric(limited["open"], errors="coerce").iloc[signal_index],
            close.iloc[signal_index - 1],
        )

    result.update(
        {
            "factor_assignment_available": True,
            "pre_entry_20d_return": primary,
            "pre_entry_5d_return": pre_entry_5d,
            "distance_to_20d_high": distance_to_high,
            "signal_day_gap": signal_day_gap,
        }
    )
    return result
...
def _factor_features_for_sources(
    ...
    for source in sources:
        ...
        if not matches:
            ...
            features[identifier] = base
            continue
        factor = _overheat_features(frame, int(matches[-1]))
    ...
    return features, {
        ...
        "primary_definition": (
            "close(signal_day)/close(signal_day-20_sessions)-1"
        ),
        "signal_day_closed_bars_only": True,
        "twenty_day_high_window": "signal_day_and_prior_19_sessions",
        "entry_timing": "T+1",
    }
...
def _assign_primary_halves(rows: list[dict[str, Any]]) -> None:
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            cooler = float(row[PRIMARY_FACTOR]) < median
            row["primary_cooler"] = cooler
            row["variant_included"] = cooler
...
def _candidate_report(
    ...
    primary_rows = [
        row for row in assigned if row.get("primary_cooler") is not None
    ]
    cooler = [row for row in primary_rows if row.get("primary_cooler")]
    hotter = [row for row in primary_rows if row.get("primary_cooler") is False]
    bootstraps = {
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_pre_entry_20d_return_lt_median_is_cooler"
        ),
        "primary_cooler_n": len(cooler),
        "primary_hotter_n": len(hotter),
        "baseline": {field: stats([row.get(field) for row in rows]) for field in FIELDS},
        "primary_cooler": {
            field: stats([row.get(field) for row in cooler]) for field in FIELDS
        },
        "primary_hotter": {
            field: stats([row.get(field) for row in hotter]) for field in FIELDS
        },
        "factors": {
            factor: _factor_report(rows, factor, spec["larger_is_better"])
            for factor, spec in FACTOR_SPECS.items()
        },
        "cluster_bootstrap_cooler_minus_hotter": bootstraps,
        "sample_gate": {
...
def _cross_split_gate(split_reports: dict[str, Any]) -> dict[str, Any]:
    ...
    directions = {
        split: {
            ...
            ["cluster_bootstrap_cooler_minus_hotter"][outcome]["mean_delta"]
            for outcome in ("future_40d", "trade_pnl_pct")
        }
        for split, report in split_reports.items()
    }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for split_index, split in enumerate(splits):
        ...
        enriched_path = output_dir / f"short_term_overheat_{split}.jsonl"
    ...
    result_path = output_dir / "short_term_overheat_audit.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default=r"D:\tmp\short_term_overheat_candidate",
    )
    parser.add_argument("--config", default=str(BASE_DIR / "config" / "config.yaml"))
    parser.add_argument("--label", default="short-term-overheat-candidate-audit")
