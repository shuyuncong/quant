"""Audit same-day candidate correlation crowding as a risk factor.

The pre-registered hypothesis is that, within a signal day, candidates with
lower mean correlation to the other candidates over the last 60 common QFQ
return intervals have better current production-risk replay outcomes.  The
primary estimator is a signal-day fixed-effect low-minus-high coefficient.
Holdout, portfolio construction, databases, network fetches, period scans,
threshold scans, and reverse-direction tests are forbidden.
"""
...
VERSION = "same_day_correlation_risk_audit.v1"
SPLITS = ("train", "val", "test")
PREREGISTERED_LABEL = "same-day-correlation60-risk-candidate-audit"
...
FORMAL_OUTPUT_DIR = Path(r"D:\tmp\same_day_correlation60_risk_candidate_final")
...
PRIMARY_FACTOR = "same_day_mean_pairwise_correlation_60"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    PRIMARY_FACTOR: {"larger_is_better": False, "role": "primary"},
    "same_day_median_pairwise_correlation_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "same_day_max_pairwise_correlation_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "same_day_positive_pair_fraction_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "same_day_valid_peer_count": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
...
COMMON_CLOSES_REQUIRED = RETURN_OBSERVATIONS + 1
MIN_VALID_PEERS = 2
...
def _validate_history_frame(frame: pd.DataFrame, path: Path) -> pd.Series:
    if not isinstance(frame, pd.DataFrame):
        raise RuntimeError(f"history is not a DataFrame: {path}")
    missing = sorted({"datetime", "close", "is_closed"} - set(frame.columns))
    if missing:
    ...
    closed = frame.loc[flags.astype(bool), ["datetime", "close"]].copy()
    timestamps = pd.to_datetime(closed["datetime"], errors="coerce")
    ...
    values = pd.to_numeric(closed["close"], errors="coerce")
    array = values.to_numpy(dtype=float)
    if not np.isfinite(array).all() or (array <= 0).any():
        raise RuntimeError(f"history close must be finite and positive: {path}")
    series = pd.Series(array, index=pd.DatetimeIndex(normalized), name="close")
    if series.index.duplicated().any():
        raise RuntimeError(f"history contains duplicate closed dates: {path}")
    if not series.index.is_monotonic_increasing:
        raise RuntimeError(f"history dates must be strictly increasing: {path}")
    return series


def _load_qfq_close(path: Path) -> pd.Series:
    try:
...
def _pair_correlation(
    ...
    common = pd.concat(
        [left.loc[:cutoff].rename("left"), right.loc[:cutoff].rename("right")],
        axis=1,
        join="inner",
    ).dropna()
    if len(common) < COMMON_CLOSES_REQUIRED:
        return None, len(common)
    window = common.tail(COMMON_CLOSES_REQUIRED)
    returns = window.pct_change(fill_method=None).iloc[1:]
    if len(returns) != RETURN_OBSERVATIONS:
    ...
    if (
        ...
        or float(np.std(left_values)) == 0.0
        or float(np.std(right_values)) == 0.0
    ):
        return None, len(window)
    correlation = float(np.corrcoef(left_values, right_values)[0, 1])
    if not math.isfinite(correlation):
        return None, len(window)
    return correlation, len(window)


def _empty_features(identity: dict[str, str], candidate_count: int) -> dict[str, Any]:
    return {
        "candidate_id": (
            f"{identity['symbol']}|{identity['signal_day']}|{identity['signal_type']}"
        ),
        **{factor: None for factor in FACTOR_SPECS},
        "same_day_candidate_count": candidate_count,
        "factor_assignment_available": False,
        "primary_low_correlation_risk": None,
        "variant_included": None,
        "pair_return_observations": RETURN_OBSERVATIONS,
        "minimum_valid_peer_symbols": MIN_VALID_PEERS,
        "entry_timing": "T+1",
    }
...
def _factor_features_for_sources(
    ...
    for identity in identities:
        features[candidate_id(identity)] = _empty_features(
            identity,
            len(by_day[identity["signal_day"]]),
        )

    valid_pairs = 0
    unavailable_pairs = 0
    common_close_counts: list[int] = []
    for signal_day, day_identities in sorted(by_day.items()):
        symbols = [identity["symbol"] for identity in day_identities]
        ...
        for symbol in symbols:
            if symbol not in history_cache:
                ...
                history_cache[symbol] = _load_qfq_close(
                    Path(str(history_item["path"]))
                )
            series = history_cache[symbol]
            if pd.Timestamp(signal_day) not in series.index:
                raise RuntimeError(
                    f"signal day is absent from closed QFQ history: "
        ...
        for left_index, left_symbol in enumerate(symbols):
            for right_symbol in symbols[left_index + 1 :]:
                if left_symbol == right_symbol:
                    raise RuntimeError(
                        f"self pair encountered for {left_symbol} on {signal_day}"
                    )
                correlation, common_count = _pair_correlation(
                    closes[left_symbol],
                    closes[right_symbol],
                    signal_day,
                )
                if correlation is None:
                    ...
                    continue
                valid_pairs += 1
                common_close_counts.append(common_count)
                peer_values[left_symbol].append(correlation)
                peer_values[right_symbol].append(correlation)

        for identity in day_identities:
            identifier = candidate_id(identity)
            values = peer_values[identity["symbol"]]
            if len(values) < MIN_VALID_PEERS:
                continue
            feature = features[identifier]
            feature.update(
                {
                    PRIMARY_FACTOR: float(np.mean(values)),
                    "same_day_median_pairwise_correlation_60": float(
                        np.median(values)
                    ),
                    "same_day_max_pairwise_correlation_60": float(max(values)),
                    "same_day_positive_pair_fraction_60": float(
                        np.mean([value > 0 for value in values])
                    ),
                    "same_day_valid_peer_count": len(values),
    ...
    for signal_day, day_identities in by_day.items():
        ...
        values = [float(item[PRIMARY_FACTOR]) for item in available]
        if len(available) < 2 or len(set(values)) < 2:
            continue
        median = float(np.median(values))
        for item in available:
            low = float(item[PRIMARY_FACTOR]) <= median
            item["primary_low_correlation_risk"] = low
    ...
    meta = {
        ...
        "unavailable_pair_count": unavailable_pairs,
        "valid_common_close_count_min": (
            min(common_close_counts) if common_close_counts else None
        ),
        "valid_common_close_count_median": (
            float(np.median(common_close_counts)) if common_close_counts else None
        ),
        "valid_common_close_count_max": (
            max(common_close_counts) if common_close_counts else None
        ),
        "primary_unavailable_candidates": unavailable_candidates,
        "primary_definition": (
            "mean_pairwise_Pearson_correlation_of_last_60_returns_formed_from_"
            "last_61_common_QFQ_closes_through_signal_day"
        ),
        "pair_nodes": "unique_symbol_per_signal_day",
        "minimum_valid_peer_symbols": MIN_VALID_PEERS,
...
def _valid_fe_rows(
    ...
    for row in rows:
        ...
        value = row[outcome]
        if value is None or row.get("primary_low_correlation_risk") is None:
            continue
        if isinstance(value, (bool, np.bool_)) or not isinstance(
            value,
            (int, float, np.integer, np.floating),
    ...
    for row in preliminary:
        labels_by_day[str(row["signal_day"])].add(
            bool(row["primary_low_correlation_risk"])
        )
...
def _fixed_effect_beta(
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            if weight <= 0:
                continue
            x_value = 1.0 if row["primary_low_correlation_risk"] is True else 0.0
...
def _outcome_sample_gate(
    ...
    low = [row for row in valid if row["primary_low_correlation_risk"] is True]
    high = [row for row in valid if row["primary_low_correlation_risk"] is False]
        ...
        reports[regime] = {
            ...
            "comparable_n": len(
                [
                    ...
                    if row.get("primary_low_correlation_risk") is not None
                ]
            ),
...
def _candidate_report(
    ...
    low = [row for row in primary_rows if row["primary_low_correlation_risk"]]
    high = [
        row for row in primary_rows if row["primary_low_correlation_risk"] is False
    ]
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_mean_pairwise_correlation_60_lte_median_is_low"
        ),
        ...
        "fixed_effect_contract": {
            ...
            "x": "1_for_low_correlation_risk_0_for_high",
...
def _write_outputs_atomically(
    ...
    for split in SPLITS:
        filename = f"same_day_correlation_risk_{split}.jsonl"
    ...
    result["report_path"] = str(output_dir / "same_day_correlation_risk_audit.json")
    report_path = staging_dir / "same_day_correlation_risk_audit.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "candidate_risk_role": "same_day_portfolio_correlation_crowding",
        ...
        "research_provenance": {
            "direction_source": "preregistered_lower_same_day_correlation_risk",
            "industry_mapping_available": False,
            "industry_density_tested": False,
            "abandoned_day_level_candidate_count_due_to_overlap_clusters": {
                "train": 1,
                "val": 1,
                "test": 2,
            },
            "outcome_free_history_precheck": {
                "train": {
                    ...
                    "available": 83,
                    "coverage": 83 / 89,
                    "comparable_candidates": 83,
                    "comparable_symbols": 80,
                    "comparable_signal_days": 11,
                },
                "val": {
                    "source": 1107,
                    "available": 1095,
                    "coverage": 1095 / 1107,
                    "comparable_candidates": 1095,
                    "comparable_symbols": 665,
                    "comparable_signal_days": 92,
                },
                "test": {
                    "source": 346,
                    "available": 342,
        ...
        "thresholds": {
            ...
            "minimum_outcome_clusters": MIN_OUTCOME_CLUSTERS,
            "common_closes": COMMON_CLOSES_REQUIRED,
            "paired_return_observations": RETURN_OBSERVATIONS,
            "minimum_valid_peer_symbols": MIN_VALID_PEERS,
        },
        "safety": {
    ...
    result["verdict"] = (
        "candidate_correlation_risk_support_requires_portfolio_preregistration"
