"""Audit 60-return QFQ price staleness as a candidate-layer risk factor.

The pre-registered hypothesis is that candidates with fewer exactly repeated
adjacent closes in the last 60 QFQ returns have better current production-risk
replay outcomes.  The primary estimator is a signal-day fixed-effect
low-minus-high coefficient.  Holdout, portfolio construction, databases,
network fetches, parameter scans, tolerance scans, and reverse tests are
forbidden.
"""
...
VERSION = "price_staleness_risk_audit.v1"
SPLITS = ("train", "val", "test")
PREREGISTERED_LABEL = "price-staleness60-risk-candidate-audit"
...
FORMAL_OUTPUT_DIR = Path(r"D:\tmp\price_staleness60_risk_candidate_final")
...
PRIMARY_FACTOR = "price_staleness_fraction_60"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    PRIMARY_FACTOR: {"larger_is_better": False, "role": "primary"},
    "max_consecutive_stale_returns_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "stale_return_episode_count_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "unique_close_fraction_61": {
        "larger_is_better": True,
        "role": "diagnostic",
    },
    "roll_spread_proxy_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "median_abs_nonzero_return_60": {
...
def _price_staleness_features(
    ...
    if len(returns) != RETURN_OBSERVATIONS or not np.isfinite(returns).all():
        raise RuntimeError(
            f"staleness return construction failed for signal_day={signal_day}"
        )

    stale = close_values[1:] == close_values[:-1]
    max_stale_run = 0
    current_stale_run = 0
    stale_episodes = 0
    for value in stale:
    ...
    lag_one_covariance = float(
        np.mean((lagged - lagged_mean) * (current - current_mean))
    )
    roll_spread_proxy = 2.0 * math.sqrt(max(-lag_one_covariance, 0.0))
    nonzero_abs_returns = np.abs(returns[returns != 0.0])
    median_abs_nonzero = (
        float(np.median(nonzero_abs_returns))
        if len(nonzero_abs_returns) > 0
        else None
    )
    result: dict[str, float | int | None] = {
        PRIMARY_FACTOR: float(np.mean(stale)),
        "max_consecutive_stale_returns_60": max_stale_run,
        "stale_return_episode_count_60": stale_episodes,
        "unique_close_fraction_61": float(
            len(np.unique(close_values)) / CLOSES_REQUIRED
        ),
        "roll_spread_proxy_60": roll_spread_proxy,
        "median_abs_nonzero_return_60": median_abs_nonzero,
    ...
    if any(
        ...
        raise RuntimeError(
            f"non-finite price staleness feature for signal_day={signal_day}: "
            f"{result}"
        )
    return result
...
def _empty_features(identity: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        **{factor: None for factor in FACTOR_SPECS},
        ...
        "primary_low_staleness_risk": None,
...
def _factor_features_for_sources(
    ...
    for identity in identities:
        ...
        computed = _price_staleness_features(series, signal_day)
        if computed is None:
    ...
    for signal_day, day_identities in by_day.items():
        ...
        for item, low in zip(available, labels):
            item["primary_low_staleness_risk"] = low
    ...
    meta = {
        ...
        "primary_definition": (
            "fraction_of_60_adjacent_closed_QFQ_close_pairs_with_"
            "exactly_equal_close_values_through_signal_day"
        ),
        "return_observations": RETURN_OBSERVATIONS,
        "close_observations": CLOSES_REQUIRED,
        "exact_zero_comparison": True,
        "near_zero_tolerance_used": False,
...
def _valid_fe_rows(
    ...
    for row in rows:
        ...
        value = row[outcome]
        if value is None or row.get("primary_low_staleness_risk") is None:
            continue
        if isinstance(value, (bool, np.bool_)) or not isinstance(
            value,
            (int, float, np.integer, np.floating),
    ...
    for row in preliminary:
        labels_by_day[str(row["signal_day"])].add(
            bool(row["primary_low_staleness_risk"])
        )
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            if weight <= 0:
                continue
            x_value = (
                1.0 if row["primary_low_staleness_risk"] is True else 0.0
            )
...
def _outcome_sample_gate(
    ...
    low = [
        row for row in valid if row["primary_low_staleness_risk"] is True
    ]
    high = [
        row for row in valid if row["primary_low_staleness_risk"] is False
    ]
        ...
        reports[regime] = {
            ...
            "comparable_n": len(
                [
                    ...
                    if row.get("primary_low_staleness_risk") is not None
                ]
            ),
...
def _candidate_report(
    ...
    low = [row for row in primary_rows if row["primary_low_staleness_risk"]]
    high = [
        row
        for row in primary_rows
        if row["primary_low_staleness_risk"] is False
    ]
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_price_staleness_fraction_60_lte_median_is_low"
        ),
        ...
        "fixed_effect_contract": {
            ...
            "x": "1_for_low_price_staleness_risk_0_for_high",
...
def _write_outputs_atomically(
    ...
    for split in SPLITS:
        filename = f"price_staleness_risk_{split}.jsonl"
    ...
    result["report_path"] = str(output_dir / "price_staleness_risk_audit.json")
    report_path = staging_dir / "price_staleness_risk_audit.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "candidate_risk_role": "qfq_adjacent_close_price_staleness_proxy",
            "primary_scope": "all_canonical",
            "primary_factor": PRIMARY_FACTOR,
            "primary_larger_is_better": False,
            "primary_comparison": "signal_day_fixed_effect_low_minus_high",
            "same_signal_day_comparison": True,
            "qfq_adjacent_close_repetition_proxy": True,
            "true_turnover_or_spread_measured": False,
            "frozen_qfq_sequence_only": True,
            "suspension_duration_measured": False,
            "time_varying_adjustment_invariance_claimed": False,
            "exact_close_equality_without_tolerance": True,
            "roll_proxy_is_diagnostic_return_covariance_measure": True,
            ...
            "zero_tolerance_scan_performed": False,
            "reverse_direction_tested": False,
        },
        "research_provenance": {
            "direction_source": "preregistered_lower_price_staleness_risk",
            ...
            "outcome_free_history_precheck": {
                ...
                "val": {
                    ...
                    "available": 1107,
                    "coverage": 1.0,
                    "comparable_candidates": 1096,
                    "comparable_symbols": 664,
                    "comparable_signal_days": 94,
                },
                "test": {
                    "source": 346,
                    "available": 346,
                    "coverage": 1.0,
                    "comparable_candidates": 335,
                    "comparable_symbols": 291,
                    "comparable_signal_days": 31,
                },
            },
    ...
    result["verdict"] = (
        "candidate_price_staleness_risk_support_requires_portfolio_preregistration"
