"""Audit the 60-session QFQ overnight squared-move share as a risk factor.

The pre-registered hypothesis is that candidates whose frozen QFQ sequence
has a lower share of squared moves occurring from prior close to current open
have better current production-risk replay outcomes.  The primary estimator
is a signal-day fixed-effect low-minus-high coefficient.  Holdout, portfolio
construction, databases, network fetches, parameter scans, and reverse tests
are forbidden.
"""
...
from backtest_winrate import HISTORY_DIR  # noqa: E402
...
VERSION = "overnight_variance_share_risk_audit.v1"
SPLITS = ("train", "val", "test")
PREREGISTERED_LABEL = "overnight-variance-share60-risk-candidate-audit"
PREREGISTERED_SEED = 20260906
...
PRIMARY_FACTOR = "overnight_variance_share_60"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    ...
    "mean_absolute_overnight_gap_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "negative_overnight_gap_fraction_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
...
MOVE_OBSERVATIONS = 60
HISTORY_BARS_REQUIRED = MOVE_OBSERVATIONS + 1
...
def _validate_history_frame(frame: pd.DataFrame, path: Path) -> pd.DataFrame:
    if not isinstance(frame, pd.DataFrame):
        raise RuntimeError(f"history is not a DataFrame: {path}")
    price_columns = ["open", "high", "low", "close"]
    required = {"datetime", "is_closed", *price_columns}
    missing = sorted(required - set(frame.columns))
    if missing:
        raise RuntimeError(f"history missing columns {missing}: {path}")
    if frame.attrs.get("adjust") != "qfq":
        raise RuntimeError(f"history adjust must be qfq: {path}")
    ...
    if not np.isfinite(prices).all() or (prices <= 0).any():
        raise RuntimeError(
            f"history OHLC must be finite and strictly positive: {path}"
        )
    open_values, high_values, low_values, close_values = prices.T
    if (
        ...
        raise RuntimeError(
            f"history OHLC relationships are invalid: {path}"
        )
...
def _load_qfq_ohlc(path: Path) -> pd.DataFrame:
    try:
        frame = pd.read_pickle(path)
    except Exception as exc:  # pragma: no cover - precise pickle errors vary
        raise RuntimeError(f"unable to read history {path}: {exc}") from exc
    return _validate_history_frame(frame, path)


def _overnight_variance_share_features(
    ohlc: pd.DataFrame,
    signal_day: str,
) -> dict[str, float | int | None] | None:
    cutoff = pd.Timestamp(signal_day)
    window = ohlc.loc[:cutoff].tail(HISTORY_BARS_REQUIRED)
    if len(window) < HISTORY_BARS_REQUIRED:
        return None
    open_values = window["open"].to_numpy(dtype=float)[1:]
    close_values = window["close"].to_numpy(dtype=float)
    prior_closes = close_values[:-1]
    current_closes = close_values[1:]
    overnight_returns = open_values / prior_closes - 1.0
    intraday_returns = current_closes / open_values - 1.0
    if (
        len(overnight_returns) != MOVE_OBSERVATIONS
        or len(intraday_returns) != MOVE_OBSERVATIONS
        or not np.isfinite(overnight_returns).all()
        or not np.isfinite(intraday_returns).all()
    ):
        raise RuntimeError(
            f"overnight/intraday return construction failed for "
            f"signal_day={signal_day}"
        )

    overnight_squared_sum = float(np.sum(np.square(overnight_returns)))
    intraday_squared_sum = float(np.sum(np.square(intraday_returns)))
    total_squared_move = overnight_squared_sum + intraday_squared_sum
    if total_squared_move == 0.0:
        return None
    result: dict[str, float | int | None] = {
        PRIMARY_FACTOR: overnight_squared_sum / total_squared_move,
        "overnight_rms_60": float(
            np.sqrt(np.mean(np.square(overnight_returns)))
        ),
        "intraday_rms_60": float(
            np.sqrt(np.mean(np.square(intraday_returns)))
        ),
        "mean_absolute_overnight_gap_60": float(
            np.mean(np.abs(overnight_returns))
        ),
        "negative_overnight_gap_fraction_60": float(
            np.mean(overnight_returns < 0.0)
        ),
        "mean_overnight_gap_60": float(np.mean(overnight_returns)),
        "overnight_squared_move_sum": overnight_squared_sum,
        "intraday_squared_move_sum": intraday_squared_sum,
        "move_observations": MOVE_OBSERVATIONS,
        "history_bars_required": HISTORY_BARS_REQUIRED,
    }
    if any(
        ...
        raise RuntimeError(
            f"non-finite overnight variance share feature for "
            f"signal_day={signal_day}: {result}"
        )
    return result
...
def _empty_features(identity: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        **{factor: None for factor in FACTOR_SPECS},
        ...
        "primary_low_overnight_variance_share_risk": None,
...
def _factor_features_for_sources(
    ...
    zero_total_squared_move_candidates = 0
    for identity in identities:
        symbol = identity["symbol"]
        if symbol not in history_cache:
            history_item = history_snapshot.get(f"qfq:{symbol}")
            if history_item is None:
                raise RuntimeError(f"history snapshot missing qfq:{symbol}")
            path = Path(str(history_item["path"]))
            history_cache[symbol] = _load_qfq_ohlc(path)
        ohlc = history_cache[symbol]
        signal_day = identity["signal_day"]
        if pd.Timestamp(signal_day) not in ohlc.index:
            raise RuntimeError(
                "signal day is absent from closed QFQ history: "
                f"symbol={symbol}, signal_day={signal_day}"
            )
        available_bars = len(
            ohlc.loc[: pd.Timestamp(signal_day)].tail(HISTORY_BARS_REQUIRED)
        )
        computed = _overnight_variance_share_features(ohlc, signal_day)
        if computed is None:
            if available_bars < HISTORY_BARS_REQUIRED:
                insufficient_history += 1
            else:
                zero_total_squared_move_candidates += 1
            continue
    ...
    for signal_day, day_identities in by_day.items():
        available = [
        ...
        for item, low in zip(available, labels):
            item["primary_low_overnight_variance_share_risk"] = low
    ...
    meta = {
        ...
        "factor_errors": {},
        "insufficient_history_candidates": insufficient_history,
        "zero_total_squared_move_candidates": (
            zero_total_squared_move_candidates
        ),
        "primary_unavailable_candidates": unavailable_candidates,
        "excluded_single_group_signal_days": excluded_single_group_days,
        "primary_definition": (
            "sum_squared_QFQ_overnight_returns_divided_by_sum_squared_"
            "overnight_plus_intraday_returns_over_60_closed_sessions"
        ),
        "move_observations": MOVE_OBSERVATIONS,
        "history_bars_required": HISTORY_BARS_REQUIRED,
        "overnight_return_definition": "open_t_div_prior_close_minus_1",
        "intraday_return_definition": "close_t_div_open_t_minus_1",
        "exact_variance_decomposition_claimed": False,
        "total_squared_move_floor_used": False,
        "qfq_uniform_scale_invariant": True,
...
def _valid_fe_rows(
    ...
    for row in rows:
        ...
        if (
            value is None
            or row.get("primary_low_overnight_variance_share_risk") is None
        ):
            continue
        if isinstance(value, (bool, np.bool_)) or not isinstance(
            value,
    ...
    for row in preliminary:
        labels_by_day[str(row["signal_day"])].add(
            bool(row["primary_low_overnight_variance_share_risk"])
        )
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            if weight <= 0:
                continue
            x_value = 1.0 if row[
                "primary_low_overnight_variance_share_risk"
            ] is True else 0.0
...
def _outcome_sample_gate(
    ...
    low = [
        row
        for row in valid
        if row["primary_low_overnight_variance_share_risk"] is True
    ]
    high = [
        row
        for row in valid
        if row["primary_low_overnight_variance_share_risk"] is False
    ]
        ...
        reports[regime] = {
            ...
            "comparable_n": len(
                [
                    ...
                    if row.get("primary_low_overnight_variance_share_risk")
                    is not None
                ]
            ),
...
def _candidate_report(
    ...
    primary_rows = [
        row
        for row in assigned
        if row.get("primary_low_overnight_variance_share_risk") is not None
    ]
    low = [
        row
        for row in primary_rows
        if row["primary_low_overnight_variance_share_risk"]
    ]
    high = [
        row
        for row in primary_rows
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_overnight_variance_share_60_"
            "lte_median_is_low"
        ),
        ...
        "fixed_effect_contract": {
            ...
            "x": "1_for_low_overnight_variance_share_risk_0_for_high",
...
def _write_outputs_atomically(
    ...
    for split in SPLITS:
        filename = f"overnight_variance_share_risk_{split}.jsonl"
    ...
    result["report_path"] = str(
        output_dir / "overnight_variance_share_risk_audit.json"
    )
    report_path = staging_dir / "overnight_variance_share_risk_audit.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "candidate_risk_role": (
                "frozen_qfq_overnight_squared_move_share_proxy"
            ),
            ...
            "squared_move_share_proxy": True,
            "exact_close_to_close_variance_decomposition": False,
            "overnight_return": "open_t_div_prior_close_minus_1",
            "intraday_return": "close_t_div_open_t_minus_1",
            "total_squared_move_zero_is_unavailable": True,
            "total_squared_move_floor_used": False,
            "frozen_qfq_sequence_only": True,
            "real_unadjusted_gap_measured": False,
            "corporate_action_loss_measured": False,
            "risk_manageability_proven": False,
            ...
            "denominator_scan_performed": False,
            "return_definition_scan_performed": False,
            "downside_only_scan_performed": False,
            "transform_scan_performed": False,
            "reverse_direction_tested": False,
        },
        "research_provenance": {
            "direction_source": (
                "preregistered_lower_overnight_variance_share_risk"
            ),
            "industry_mapping_available": False,
            "industry_density_tested": False,
            "abandoned_volume_stability_design": {
                "stopped_before_implementation": True,
                "reason": "within_file_volume_unit_continuity_not_verified",
            },
            ...
            "outcome_free_pooled_pearson": {
                "atr20_ratio": 0.17627275945699805,
                "downside_tail_es_10pct_60": 0.3479041384957498,
                "volatility_instability_cv_6x10": 0.3174763159410565,
                "price_staleness_fraction_60": -0.08140385003043876,
                "downside_gap_semideviation_60": 0.5199143929650063,
            },
        ...
        "thresholds": {
            ...
            "intraday_return_definition": "close_t_div_open_t_minus_1",
            "primary_formula": (
                "sum(overnight_return_squared)_div_"
                "sum(overnight_plus_intraday_return_squared)"
            ),
            "exact_variance_decomposition_claimed": False,
            "total_squared_move_zero_rule": "candidate_unavailable",
            "total_squared_move_floor": None,
        },
        "safety": {
    ...
    result["verdict"] = (
        "candidate_overnight_variance_share_risk_support_requires_"
