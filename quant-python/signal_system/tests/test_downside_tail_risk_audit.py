...
import downside_tail_risk_audit as audit  # noqa: E402
...
def test_contract_constants_are_frozen():
    assert audit.PRIMARY_FACTOR == "downside_tail_es_10pct_60"
    assert audit.RETURN_OBSERVATIONS == 60
    assert audit.CLOSES_REQUIRED == 61
    assert audit.TAIL_LOSS_COUNT == 6
    assert audit.PREREGISTERED_SEED == 20260904
    assert audit.PREREGISTERED_BOOTSTRAP_REPS == 2000
    assert audit.FACTOR_SPECS[audit.PRIMARY_FACTOR] == {
        "larger_is_better": False,
        "role": "primary",
    }
    assert audit.FACTOR_SPECS["return_skewness_60"] == {
...
def test_tail_es_uses_exact_largest_six_losses_and_zero_padding():
    returns = np.r_[
        [-0.10, -0.08, -0.06, -0.04, -0.02, -0.01],
        np.full(54, 0.005),
    ]
    series = _close_series(returns)
    result = audit._tail_risk_features(
        series, series.index[-1].date().isoformat()
    )
    assert result is not None
    assert result[audit.PRIMARY_FACTOR] == pytest.approx(
        (0.10 + 0.08 + 0.06 + 0.04 + 0.02 + 0.01) / 6
    )

    one_loss_returns = np.r_[-0.12, np.full(59, 0.002)]
    one_loss = _close_series(one_loss_returns)
    one_loss_result = audit._tail_risk_features(
        one_loss, one_loss.index[-1].date().isoformat()
    )
    assert one_loss_result is not None
    assert one_loss_result[audit.PRIMARY_FACTOR] == pytest.approx(0.12 / 6)

    positive = _close_series(np.full(60, 0.001))
    positive_result = audit._tail_risk_features(
        positive, positive.index[-1].date().isoformat()
    )
    assert positive_result is not None
    assert positive_result[audit.PRIMARY_FACTOR] == pytest.approx(0.0)


def test_tail_diagnostics_match_manual_formulas():
    ...
    running_peaks = np.maximum.accumulate(series.to_numpy(dtype=float))
    expected_drawdown = np.max(
        np.maximum(1.0 - series.to_numpy(dtype=float) / running_peaks, 0.0)
    )
    std = float(np.std(returns, ddof=0))
    expected_skew = float(
        np.mean((returns - float(np.mean(returns))) ** 3) / (std**3)
    )
    assert result["worst_daily_loss_60"] == pytest.approx(np.max(losses))
    assert result["downside_semideviation_60"] == pytest.approx(
        math.sqrt(float(np.mean(np.minimum(returns, 0.0) ** 2)))
    )
    assert result["negative_return_fraction_60"] == pytest.approx(
        np.mean(returns < 0)
    )
    assert result["max_drawdown_60"] == pytest.approx(expected_drawdown)
    assert result["return_skewness_60"] == pytest.approx(expected_skew)


def test_tail_features_are_qfq_scale_invariant_and_ignore_post_signal_suffix():
    returns = np.sin(np.arange(80) / 3.0) * 0.02
    series = _close_series(returns)
    day = series.index[65].date().isoformat()
    expected = audit._tail_risk_features(series, day)
    scaled = audit._tail_risk_features(series * 37.5, day)
    modified = series.copy()
    modified.iloc[70:] *= np.linspace(2.0, 9.0, len(modified.iloc[70:]))
    suffix_changed = audit._tail_risk_features(modified, day)
    assert scaled == pytest.approx(expected)
    assert suffix_changed == pytest.approx(expected)


def test_tail_features_are_unavailable_with_only_60_closes():
    series = _close_series(np.linspace(-0.01, 0.01, 59))
    assert audit._tail_risk_features(
        series, series.index[-1].date().isoformat()
    ) is None


def _feature_cache(losses: dict[str, float]) -> tuple[
    list[dict[str, object]],
    dict[str, pd.Series],
    dict[str, dict[str, object]],
]:
    day = pd.bdate_range("2025-12-01", periods=61)[-1].date().isoformat()
    sources = [_source(symbol, day) for symbol in losses]
    cache = {
        symbol: _close_series(
            np.r_[-loss, np.full(59, 0.001)]
        )
...
def test_factor_features_compute_tail_statistics_and_same_day_groups():
    ...
    assert sum(
        item["primary_low_tail_risk"] is True for item in features.values()
    ) == 2
    assert sum(
        item["primary_low_tail_risk"] is False for item in features.values()
    ) == 2
    assert meta["return_observations"] == 60
    assert meta["tail_loss_count"] == 6
    assert meta["primary_unavailable_candidates"] == 0


def test_factor_features_ignore_source_legacy_outcomes():
    sources, cache, snapshot = _feature_cache(
        {"000001": 0.01, "000002": 0.02, "000003": 0.03}
    )
    changed = [
...
def test_median_tie_that_leaves_one_side_empty_is_not_comparable():
    sources, cache, snapshot = _feature_cache(
        {"000001": 0.06, "000002": 0.12, "000003": 0.12}
    )
    features, meta = audit._factor_features_for_sources(
        sources, cache, snapshot
    )
    ...
    assert all(
        item["primary_low_tail_risk"] is None for item in features.values()
    )
...
def test_skewness_is_unoriented_and_cannot_generate_directional_reports():
    rows = [
        {
            **_row("000001", "2026-04-01", True, 1.0),
            "return_skewness_60": 9.0,
        },
        {
    ...
    report = audit._factor_report(
        rows,
        "return_skewness_60",
        audit.FACTOR_SPECS["return_skewness_60"]["larger_is_better"],
    )
...
def test_cluster_bootstrap_is_deterministic_and_uses_fixed_seed():
    ...
    first = audit._cluster_bootstrap_fe_low_minus_high(
        rows, "future_40d", reps=100, seed=20260904
    )
...
def test_fe_and_bootstrap_are_invariant_to_input_order():
    ...
    first = audit._cluster_bootstrap_fe_low_minus_high(
        rows, "future_40d", reps=50, seed=20260904
    )
    second = audit._cluster_bootstrap_fe_low_minus_high(
        list(reversed(rows)), "future_40d", reps=50, seed=20260904
    )
    assert first == second
