"""Candidate-layer audit of medium-term pre-entry momentum.

The pre-declared primary hypothesis is that a higher 60-session return through
the closed signal day should improve future and realized trade returns.  The
20-session return, 20-vs-60 log-return acceleration, and 60-session positive
return ratio are diagnostics only.  Comparisons are formed within each signal
day, canonical development candidates are required, holdout is rejected, and
no portfolio layer is implemented.
"""
...
VERSION = "medium_term_momentum_audit.v1"
SPLITS = ("train", "val", "test")
PRIMARY_FACTOR = "pre_entry_60d_return"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    "pre_entry_60d_return": {"larger_is_better": True, "role": "primary"},
    "pre_entry_20d_return": {"larger_is_better": True, "role": "diagnostic"},
...
def _momentum_features(
    ...
    """Compute causal medium-term momentum through one closed signal bar."""
    result: dict[str, Any] = {factor: None for factor in FACTOR_SPECS}
    result.update(
        {
            "factor_assignment_available": False,
            "factor_error": None,
            "momentum_lookback_bars": signal_index + 1,
            "entry_timing": "T+1",
        }
    )
    if signal_index < 0 or frame.empty or "close" not in frame.columns:
        ...
        return result
    if signal_index < 60:
        result["factor_error"] = "insufficient_60_session_return_history"
        return result

    limited = frame.iloc[: signal_index + 1]
    close = pd.to_numeric(limited["close"], errors="coerce")
    current_close = close.iloc[signal_index]
    base_60 = close.iloc[signal_index - 60]
    primary = _safe_return(current_close, base_60)
    if primary is None:
        result["factor_error"] = "invalid_pre_entry_60d_return"
        return result

    base_20 = close.iloc[signal_index - 20]
    return_20 = _safe_return(current_close, base_20)
    acceleration = None
    if return_20 is not None:
        acceleration = (
            math.log(float(current_close) / float(base_20)) / 20.0
            - math.log(float(current_close) / float(base_60)) / 60.0
        )

    positive_ratio = None
    price_window = close.iloc[signal_index - 60 : signal_index + 1]
    if (
        len(price_window) == 61
        and price_window.notna().all()
        and np.isfinite(price_window.astype(float)).all()
        and (price_window.astype(float) > 0).all()
    ):
        interval_returns = price_window.astype(float).pct_change().iloc[1:]
        positive_ratio = float((interval_returns > 0).mean())

    result.update(
        {
            "factor_assignment_available": True,
            "pre_entry_60d_return": primary,
            "pre_entry_20d_return": return_20,
            "momentum_acceleration_20_vs_60": acceleration,
            "positive_return_ratio_60": positive_ratio,
        }
    )
    return result
...
def _factor_features_for_sources(
    ...
    return features, {
        ...
        "acceleration_definition": (
            "log(close_t/close_t-20)/20-log(close_t/close_t-60)/60"
        ),
        "positive_return_ratio_definition": (
            "positive_close_to_close_intervals/60_over_t-60_through_t"
        ),
        "pre_entry_20d_reuse_policy": (
            "context_only; cannot replace or reverse the completed short-term "
            "overheat primary test"
        ),
...
def _assign_primary_halves(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        row["primary_high_momentum"] = None
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            row["primary_high_momentum"] = high
...
def _candidate_report(
    ...
    primary_rows = [
        row for row in assigned if row.get("primary_high_momentum") is not None
    ]
    high = [row for row in primary_rows if row.get("primary_high_momentum")]
    low = [
        row for row in primary_rows if row.get("primary_high_momentum") is False
    ]
    bootstraps = {
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_pre_entry_60d_return_gte_median_is_high"
        ),
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "pre_entry_20d_context_only": True,
    ...
    for split_index, split in enumerate(splits):
        ...
        enriched_path = output_dir / f"medium_term_momentum_{split}.jsonl"
    ...
    result_path = output_dir / "medium_term_momentum_audit.json"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default=r"D:\tmp\medium_term_momentum_candidate",
    )
    parser.add_argument("--config", default=str(BASE_DIR / "config" / "config.yaml"))
    parser.add_argument("--label", default="medium-term-momentum-candidate-audit")
