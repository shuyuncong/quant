"""Audit 60-return downside tail loss as a candidate-layer risk factor.

The pre-registered hypothesis is that candidates with a lower mean of the six
largest non-negative close-to-close losses from the last 60 QFQ returns have
better current production-risk replay outcomes.  The primary estimator is a
signal-day fixed-effect low-minus-high coefficient.  Holdout, portfolio
...
VERSION = "downside_tail_risk_audit.v1"
SPLITS = ("train", "val", "test")
PREREGISTERED_LABEL = "downside-tail60-risk-candidate-audit"
PREREGISTERED_SEED = 20260904
...
PRIMARY_FACTOR = "downside_tail_es_10pct_60"
FACTOR_SPECS: dict[str, dict[str, Any]] = {
    ...
    "downside_semideviation_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "negative_return_fraction_60": {
        "larger_is_better": False,
        "role": "diagnostic",
    },
    "max_drawdown_60": {
...
def _tail_risk_features(
    ...
    if len(returns) != RETURN_OBSERVATIONS or not np.isfinite(returns).all():
        raise RuntimeError(
            f"tail return construction failed for signal_day={signal_day}"
        )

    losses = np.maximum(-returns, 0.0)
    tail_losses = np.sort(losses)[-TAIL_LOSS_COUNT:]
    downside_semideviation = float(
        math.sqrt(float(np.mean(np.minimum(returns, 0.0) ** 2)))
    )
    running_peaks = np.maximum.accumulate(close_values)
    drawdowns = np.maximum(1.0 - close_values / running_peaks, 0.0)
    std = float(np.std(returns, ddof=0))
    skewness = (
        float(np.mean((returns - float(np.mean(returns))) ** 3) / (std**3))
        if std > 0
        else None
    )
    result: dict[str, float | int | None] = {
        PRIMARY_FACTOR: float(np.mean(tail_losses)),
        "worst_daily_loss_60": float(np.max(losses)),
        "downside_semideviation_60": downside_semideviation,
        "negative_return_fraction_60": float(np.mean(returns < 0)),
        "max_drawdown_60": float(np.max(drawdowns)),
        "return_skewness_60": skewness,
    ...
    if any(
        ...
        raise RuntimeError(
            f"non-finite tail risk feature for signal_day={signal_day}: {result}"
        )
    return result
...
def _factor_features_for_sources(
    ...
    for identity in identities:
        ...
        computed = _tail_risk_features(series, signal_day)
        if computed is None:
    ...
    for signal_day, day_identities in by_day.items():
        ...
        for item, low in zip(available, labels):
            item["primary_low_tail_risk"] = low
    ...
    meta = {
        ...
        "primary_definition": (
            "mean_of_largest_6_nonnegative_losses_from_60_simple_returns_"
            "formed_by_last_61_closed_QFQ_closes_through_signal_day"
        ),
        "return_observations": RETURN_OBSERVATIONS,
        "tail_loss_count": TAIL_LOSS_COUNT,
        "close_observations": CLOSES_REQUIRED,
        "qfq_scale_invariant": True,
...
def _valid_fe_rows(
    ...
    for row in rows:
        ...
        value = row[outcome]
        if value is None or row.get("primary_low_tail_risk") is None:
            continue
        if isinstance(value, (bool, np.bool_)) or not isinstance(
            value,
            (int, float, np.integer, np.floating),
    ...
    for row in preliminary:
        labels_by_day[str(row["signal_day"])].add(
            bool(row["primary_low_tail_risk"])
        )
...
def _fixed_effect_beta(
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            if weight <= 0:
                continue
            x_value = 1.0 if row["primary_low_tail_risk"] is True else 0.0
...
def _outcome_sample_gate(
    ...
    low = [row for row in valid if row["primary_low_tail_risk"] is True]
    high = [row for row in valid if row["primary_low_tail_risk"] is False]
        ...
        reports[regime] = {
            ...
            "comparable_n": len(
                [
                    ...
                    if row.get("primary_low_tail_risk") is not None
                ]
            ),
...
def _candidate_report(
    ...
    low = [row for row in primary_rows if row["primary_low_tail_risk"]]
    high = [
        row for row in primary_rows if row["primary_low_tail_risk"] is False
    ]
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_downside_tail_es_10pct_60_lte_median_is_low"
        ),
        ...
        "fixed_effect_contract": {
            ...
            "x": "1_for_low_downside_tail_risk_0_for_high",
...
def _write_outputs_atomically(
    ...
    for split in SPLITS:
        filename = f"downside_tail_risk_{split}.jsonl"
    ...
    result["report_path"] = str(output_dir / "downside_tail_risk_audit.json")
    report_path = staging_dir / "downside_tail_risk_audit.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "candidate_risk_role": "close_return_downside_tail_risk",
            "primary_scope": "all_canonical",
            "primary_factor": PRIMARY_FACTOR,
            "primary_larger_is_better": False,
            "primary_comparison": "signal_day_fixed_effect_low_minus_high",
            "same_signal_day_comparison": True,
            "qfq_adjusted_return_tail_risk": True,
            "unadjusted_price_jump_reconstructed": False,
            ...
            "tail_fraction_scan_performed": False,
            "reverse_direction_tested": False,
        },
        "research_provenance": {
            "direction_source": "preregistered_lower_downside_tail_risk",
    ...
    result["verdict"] = (
        "candidate_downside_tail_risk_support_requires_portfolio_preregistration"
