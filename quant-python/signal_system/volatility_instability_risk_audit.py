"""Audit 6x10-return volatility instability as a candidate risk factor.

The pre-registered hypothesis is that candidates with a lower coefficient of
variation across six chronological non-overlapping 10-return QFQ RMS blocks
...
construction, databases, network fetches, block scans, parameter scans, and
reverse tests are forbidden.
"""
...
VERSION = "volatility_instability_risk_audit.v1"
SPLITS = ("train", "val", "test")
PREREGISTERED_LABEL = "volatility-instability60-risk-candidate-audit"
PREREGISTERED_SEED = 20260905
PREREGISTERED_BOOTSTRAP_REPS = 2000
CANONICAL_INPUT_DIR = Path(r"D:\tmp\candidates_fullpool_canonical")
FORMAL_OUTPUT_DIR = Path(
    r"D:\tmp\volatility_instability60_risk_candidate_final"
)
...
BLOCK_COUNT = 6
BLOCK_RETURN_OBSERVATIONS = 10
CLOSES_REQUIRED = RETURN_OBSERVATIONS + 1
...
def _validate_history_frame(frame: pd.DataFrame, path: Path) -> pd.Series:
    ...
    if not all(
        ...
        raise RuntimeError(f"history close contains non-numeric values: {path}")
    values = raw_close.to_numpy(dtype=float)
...
def _volatility_instability_features(
    close: pd.Series,
    signal_day: str,
) -> dict[str, float | int | None] | None:
    cutoff = pd.Timestamp(signal_day)
    window = close.loc[:cutoff].tail(CLOSES_REQUIRED)
    if len(window) < CLOSES_REQUIRED:
        return None
    close_values = window.to_numpy(dtype=float)
    returns = close_values[1:] / close_values[:-1] - 1.0
    if len(returns) != RETURN_OBSERVATIONS or not np.isfinite(returns).all():
        raise RuntimeError(
            f"volatility return construction failed for signal_day={signal_day}"
        )

    blocks = returns.reshape(BLOCK_COUNT, BLOCK_RETURN_OBSERVATIONS)
    block_rms = np.sqrt(np.mean(np.square(blocks), axis=1))
    mean_block_rms = float(np.mean(block_rms))
    if mean_block_rms == 0.0:
        return None
    block_rms_std = float(np.std(block_rms, ddof=0))
    block_positions = np.arange(BLOCK_COUNT, dtype=float)
    centered_positions = block_positions - float(np.mean(block_positions))
    trend_denominator = float(np.sum(np.square(centered_positions)))
    trend_slope = float(
        np.sum(centered_positions * (block_rms - mean_block_rms))
        / trend_denominator
    )
    result: dict[str, float | int | None] = {
        PRIMARY_FACTOR: block_rms_std / mean_block_rms,
        "mean_block_rms_10_60": mean_block_rms,
        "max_block_rms_10_60": float(np.max(block_rms)),
        "block_rms_range_60": float(np.max(block_rms) - np.min(block_rms)),
        "latest_block_rms_10": float(block_rms[-1]),
        "block_rms_trend_slope_60": trend_slope,
        "return_observations": RETURN_OBSERVATIONS,
        "block_count": BLOCK_COUNT,
        "block_return_observations": BLOCK_RETURN_OBSERVATIONS,
        "close_observations": CLOSES_REQUIRED,
    }
    if any(
        ...
        raise RuntimeError(
            f"non-finite volatility instability feature for "
...
def _empty_features(identity: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        **{factor: None for factor in FACTOR_SPECS},
        ...
        "primary_low_volatility_instability_risk": None,
...
def _factor_features_for_sources(
    ...
    zero_mean_block_rms_candidates = 0
    for identity in identities:
        ...
        if symbol not in history_cache:
            ...
            history_cache[symbol] = _load_qfq_close(path)
        ...
        available_closes = len(
            series.loc[: pd.Timestamp(signal_day)].tail(CLOSES_REQUIRED)
        )
        computed = _volatility_instability_features(series, signal_day)
        if computed is None:
            if available_closes < CLOSES_REQUIRED:
                insufficient_history += 1
            else:
                zero_mean_block_rms_candidates += 1
            continue
    ...
    for signal_day, day_identities in by_day.items():
        ...
        for item, low in zip(available, labels):
            item["primary_low_volatility_instability_risk"] = low
    ...
    meta = {
        ...
        "zero_mean_block_rms_candidates": zero_mean_block_rms_candidates,
        ...
        "primary_definition": (
            ...
            "nonoverlapping_10_return_RMS_blocks_from_last_60_simple_returns"
        ),
        "return_observations": RETURN_OBSERVATIONS,
        "block_count": BLOCK_COUNT,
        "block_return_observations": BLOCK_RETURN_OBSERVATIONS,
        ...
        "block_rms_demeaned": False,
        "block_rms_annualized": False,
        "cv_std_ddof": 0,
        "mean_rms_floor_used": False,
...
def _valid_fe_rows(
    ...
    for row in rows:
        ...
        if (
            value is None
            or row.get("primary_low_volatility_instability_risk") is None
        ):
            continue
        if isinstance(value, (bool, np.bool_)) or not isinstance(
            value,
    ...
    for row in preliminary:
        labels_by_day[str(row["signal_day"])].add(
            bool(row["primary_low_volatility_instability_risk"])
        )
    ...
    for day_rows in by_day.values():
        ...
        for row in day_rows:
            ...
            if weight <= 0:
                continue
            x_value = 1.0 if row[
                "primary_low_volatility_instability_risk"
            ] is True else 0.0
...
def _outcome_sample_gate(
    ...
    low = [
        row
        for row in valid
        if row["primary_low_volatility_instability_risk"] is True
    ]
    high = [
        row
        for row in valid
        if row["primary_low_volatility_instability_risk"] is False
    ]
        ...
        reports[regime] = {
            ...
            "comparable_n": len(
                [
                    ...
                    if row.get("primary_low_volatility_instability_risk")
                    is not None
                ]
            ),
...
def _candidate_report(
    ...
    primary_rows = [
        row
        for row in assigned
        if row.get("primary_low_volatility_instability_risk") is not None
    ]
    low = [
        row
        for row in primary_rows
        if row["primary_low_volatility_instability_risk"]
    ]
    high = [
        row
        for row in primary_rows
    ...
    return {
        ...
        "primary_group_rule": (
            "same_signal_day_volatility_instability_cv_6x10_"
            "lte_median_is_low"
        ),
        ...
        "fixed_effect_contract": {
            ...
            "x": "1_for_low_volatility_instability_risk_0_for_high",
...
def _write_outputs_atomically(
    ...
    for split in SPLITS:
        filename = f"volatility_instability_risk_{split}.jsonl"
    ...
    result["report_path"] = str(
        output_dir / "volatility_instability_risk_audit.json"
    )
    report_path = staging_dir / "volatility_instability_risk_audit.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "design": {
            ...
            "candidate_risk_role": (
                "qfq_six_block_return_rms_instability_proxy"
            ),
            ...
            "qfq_six_block_rms_cv_proxy": True,
            "fixed_chronological_6x10_partition": True,
            "nonoverlapping_return_blocks": True,
            "block_rms_uses_raw_simple_returns": True,
            "block_rms_demeaned": False,
            "block_rms_annualized": False,
            "cv_population_std_ddof": 0,
            "mean_block_rms_zero_is_unavailable": True,
            "mean_block_rms_floor_used": False,
        ...
        "research_provenance": {
            "direction_source": (
                "preregistered_lower_volatility_instability_risk"
            ),
        ...
        "thresholds": {
            ...
            "close_observations": CLOSES_REQUIRED,
            "return_observations": RETURN_OBSERVATIONS,
            "return_definition": "simple_close_to_close_return",
            "block_rms_formula": "sqrt(mean(return_squared))",
            "block_rms_demeaned": False,
            "block_rms_annualized": False,
            "block_count": BLOCK_COUNT,
            "block_return_observations": BLOCK_RETURN_OBSERVATIONS,
            ...
            "blocks_overlap": False,
            "cv_std_ddof": 0,
            "mean_rms_zero_rule": "candidate_unavailable",
            "mean_rms_floor": None,
        },
        "safety": {
    ...
    result["verdict"] = (
        "candidate_volatility_instability_risk_support_requires_"
