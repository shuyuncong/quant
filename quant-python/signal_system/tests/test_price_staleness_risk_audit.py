...
import price_staleness_risk_audit as audit  # noqa: E402
...
def _row(
    ...
    row = {
        **_source(symbol, day),
        ...
        "primary_low_staleness_risk": low,
...
def test_contract_constants_are_frozen():
    assert audit.PRIMARY_FACTOR == "price_staleness_fraction_60"
    ...
    assert audit.FACTOR_SPECS["median_abs_nonzero_return_60"] == {
...
def test_price_staleness_uses_exact_repeated_adjacent_closes():
    returns = np.full(60, 0.005)
    returns[[0, 1, 4, 6, 7, 8]] = 0.0
    series = _close_series(returns)
    result = audit._price_staleness_features(
        series, series.index[-1].date().isoformat()
    )
    assert result is not None
    assert result[audit.PRIMARY_FACTOR] == pytest.approx(6 / 60)
    assert result["max_consecutive_stale_returns_60"] == 3
    assert result["stale_return_episode_count_60"] == 3
    assert result["unique_close_fraction_61"] == pytest.approx(55 / 61)


def test_staleness_diagnostics_match_manual_formulas():
    ...
    lagged = actual_returns[:-1]
    current = actual_returns[1:]
    covariance = float(
        np.mean(
            (lagged - float(np.mean(lagged)))
            * (current - float(np.mean(current)))
        )
    )
    expected_roll = 2.0 * math.sqrt(max(-covariance, 0.0))
    nonzero = np.abs(actual_returns[actual_returns != 0.0])
    assert result["roll_spread_proxy_60"] == pytest.approx(expected_roll)
    assert result["median_abs_nonzero_return_60"] == pytest.approx(
        np.median(nonzero)
    )


def test_roll_spread_proxy_positive_branch_uses_negative_return_covariance():
    returns = np.tile(np.array([0.02, -0.02]), 30)
    series = _close_series(returns)
    result = audit._price_staleness_features(
        series, series.index[-1].date().isoformat()
    )
    assert result is not None
    actual_returns = series.to_numpy()[1:] / series.to_numpy()[:-1] - 1.0
    lagged = actual_returns[:-1]
    current = actual_returns[1:]
    covariance = float(
        np.mean(
    ...
    assert covariance < 0
    assert result["roll_spread_proxy_60"] == pytest.approx(
        2.0 * math.sqrt(-covariance)
    )
    assert result["roll_spread_proxy_60"] > 0


def test_unique_close_fraction_counts_nonadjacent_repeated_prices():
    closes = np.arange(1.0, 62.0)
    closes[2] = closes[0]
    series = pd.Series(
        closes,
        index=pd.bdate_range("2025-12-01", periods=61),
        name="close",
    )
    result = audit._price_staleness_features(
        series, series.index[-1].date().isoformat()
    ...
    assert result is not None
    assert result[audit.PRIMARY_FACTOR] == pytest.approx(0.0)
    assert result["unique_close_fraction_61"] == pytest.approx(60 / 61)


def test_all_stale_window_has_one_episode_and_no_nonzero_median():
    series = _close_series(np.zeros(60))
    result = audit._price_staleness_features(
        series, series.index[-1].date().isoformat()
    )
    assert result is not None
    assert result[audit.PRIMARY_FACTOR] == pytest.approx(1.0)
    assert result["max_consecutive_stale_returns_60"] == 60
    assert result["stale_return_episode_count_60"] == 1
    assert result["unique_close_fraction_61"] == pytest.approx(1 / 61)
    assert result["roll_spread_proxy_60"] == pytest.approx(0.0)
    assert result["median_abs_nonzero_return_60"] is None


def test_nextafter_change_is_non_stale_without_rounding_or_tolerance():
    closes = np.ones(61, dtype=float)
    closes[1] = np.nextafter(closes[0], 2.0)
    series = pd.Series(
        closes,
        index=pd.bdate_range("2025-12-01", periods=61),
        name="close",
    ...
    assert result is not None
    assert result[audit.PRIMARY_FACTOR] == pytest.approx(58 / 60)


def test_staleness_features_are_scale_invariant_and_ignore_post_signal_suffix():
    returns = np.sin(np.arange(80) / 3.0) * 0.02
    returns[[1, 5, 6, 22, 45, 46, 47, 70]] = 0.0
    series = _close_series(returns)
    day = series.index[65].date().isoformat()
    expected = audit._price_staleness_features(series, day)
    scaled = audit._price_staleness_features(series * 8.0, day)
    modified = series.copy()
    modified.iloc[70:] *= np.linspace(2.0, 9.0, len(modified.iloc[70:]))
    suffix_changed = audit._price_staleness_features(modified, day)
    assert scaled == pytest.approx(expected)
    assert suffix_changed == pytest.approx(expected)


def test_staleness_features_are_unavailable_with_only_60_closes():
    series = _close_series(np.linspace(-0.01, 0.01, 59))
    assert audit._price_staleness_features(
        series, series.index[-1].date().isoformat()
    ) is None


def _feature_cache(stale_counts: dict[str, int]) -> tuple[
    list[dict[str, object]],
    dict[str, pd.Series],
    dict[str, dict[str, object]],
]:
    day = pd.bdate_range("2025-12-01", periods=61)[-1].date().isoformat()
    sources = [_source(symbol, day) for symbol in stale_counts]
    cache = {
        symbol: _close_series(
            np.r_[np.zeros(stale_count), np.full(60 - stale_count, 0.001)]
        )
        for symbol, stale_count in stale_counts.items()
    }
    snapshot = {
        f"qfq:{symbol}": {
            "path": f"{symbol}.pkl",
        ...
        for symbol in stale_counts
    }
    return sources, cache, snapshot


def test_factor_features_compute_staleness_and_same_day_groups():
    ...
    assert sum(
        item["primary_low_staleness_risk"] is True for item in features.values()
    ) == 2
    assert sum(
        item["primary_low_staleness_risk"] is False for item in features.values()
    ) == 2
    assert meta["return_observations"] == 60
    assert meta["exact_zero_comparison"] is True
    assert meta["near_zero_tolerance_used"] is False
...
def test_median_tie_that_leaves_one_side_empty_is_not_comparable():
    ...
    assert all(
        item["primary_low_staleness_risk"] is None for item in features.values()
    )
...
def test_nonzero_return_median_is_unoriented_and_has_no_directional_reports():
    rows = [
        {
            **_row("000001", "2026-04-01", True, 1.0),
            "median_abs_nonzero_return_60": 0.09,
        },
        {
            **_row("000002", "2026-04-01", False, -1.0),
            "median_abs_nonzero_return_60": 0.01,
        },
    ]
    report = audit._factor_report(
        rows,
        "median_abs_nonzero_return_60",
        audit.FACTOR_SPECS["median_abs_nonzero_return_60"][
            "larger_is_better"
        ],
    )
...
def test_atomic_writer_publishes_and_records_prepublish_snapshot(
    ...
    assert {path.name for path in final.iterdir()} == {
        "price_staleness_risk_audit.json",
        "price_staleness_risk_train.jsonl",
        "price_staleness_risk_val.jsonl",
        "price_staleness_risk_test.jsonl",
    }
        ...
        return (
            {
                audit.candidate_id(row): {
                    ...
                    "factor_assignment_available": True,
                    "primary_low_staleness_risk": True,
                }
                for row in sources
            },
